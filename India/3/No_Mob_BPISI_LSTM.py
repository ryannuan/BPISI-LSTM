...
def loss_res(epochs, train_loss, val_loss, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/3/No_Mob_BPISI_LSTM/loss_' + country +
...
def plot_infect_rate(infect_rate_pred, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/3/No_Mob_BPISI_LSTM/infection_rate_' + country +
                '_' + str(learning_rate) + '_' + str(hidden_num) + '_' + str(epochs) + '_' + str(window_backward) + '.png')
...
def plot_confirmed(test_pred, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/3/No_Mob_BPISI_LSTM/confirmed_delta_' + country +
                '_' + str(learning_rate) + '_' + str(hidden_num) + '_' + str(epochs) + '_' + str(window_backward) + '.png')
...
if __name__ == '__main__':
    ...
    for ind, df in zip(inds, dfs):
        ...
        temp = []
        for i in range(6):
            temp.append(df.iloc[:-(ind + uncertain_day - 3 * i), 2:-1].values.reshape(-1, n_features))
    ...
    with open('../result/figure/India/3/No_Mob_BPISI_LSTM/' + country + '_model_res.txt', 'a') as file:
