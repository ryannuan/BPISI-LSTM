...
def merge_hist(mobility_info, disease_info, country_name, mask_ind):
    ...
    '''
    if mask_ind == None:
        disease_pre = pd.read_csv('../result/disease_pre/India/5/' + country_name + '.csv', header=0)
    else:
        disease_pre = pd.read_csv('../result/disease_pre/India/5/' + country_name + '_' + str(mask_ind) + '.csv', header=0)
