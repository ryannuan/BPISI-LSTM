...
def loss_res(epochs, train_loss, val_loss, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/5/No_Mob_BPISI_LSTM/loss_' + country +
...
def plot_infect_rate(infect_rate_pred, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/5/No_Mob_BPISI_LSTM/infection_rate_' + country +
                '_' + str(learning_rate) + '_' + str(hidden_num) + '_' + str(epochs) + '_' + str(window_backward) + '.png')
...
def plot_confirmed(test_pred, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/5/No_Mob_BPISI_LSTM/confirmed_delta_' + country +
                '_' + str(learning_rate) + '_' + str(hidden_num) + '_' + str(epochs) + '_' + str(window_backward) + '.png')
...
if __name__ == '__main__':
    ...
    for ind, df in zip(inds, dfs):
        ...
        temp = []
        for i in range(4):
            temp.append(df.iloc[:-(ind + uncertain_day - 5 * i), 2:-1].values.reshape(-1, n_features))
        data_set.append(temp)
    # data normalization
    test_X_set = [[min_max_scaler.transform(data[i]) for i in range(4)] for data in data_set]
    # predict the infection rate
    test_pred_set = [[model_test(LSTM, data_scaled[i]) for i in range(4)] for data_scaled in test_X_set]
    ...
    disease_pre = [pd.read_csv('../result/disease_pre/India/5/' + country + '_' + str(ind) + '.csv', header=0) for ind in inds]
    ...
    with open('../result/figure/India/5/No_Mob_BPISI_LSTM/' + country + '_model_res.txt', 'a') as file:
