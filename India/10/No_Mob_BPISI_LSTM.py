...
import No_Mob_BPISI_LSTM_DataLoad as preprocess
...
def loss_res(epochs, train_loss, val_loss, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/10/No_Mob_BPISI_LSTM/loss_' + country +
...
def plot_infect_rate(infect_rate_pred, learning_rate, hidden_num):
    infect_rate_real = disease_pre[0]['infection_rate_10'][drop_ind:-test_num].values.reshape(-1, 1)
    plt.clf()
    plt.plot(list(range(window_backward, window_backward+len(infect_rate_pred))),
             infect_rate_pred, c='b', label='No_Mob_BPISI_LSTM Fitting')
    plt.scatter(list(range(len(infect_rate_real))), infect_rate_real, c='r', label='Real', s=5)
    plt.legend(['No_Mob_BPISI_LSTM Fitting', 'Real'], loc="upper right")
    plt.savefig('../result/figure/India/10/No_Mob_BPISI_LSTM/infection_rate_' + country +
                '_' + str(learning_rate) + '_' + str(hidden_num) + '_' + str(epochs) + '_' + str(window_backward) + '.png')
...
def plot_confirmed(test_pred, learning_rate, hidden_num):
    ...
    plt.plot(list(range((len(real)-(30+1)), len(real))), test_pred_merge,
             c='orange', label='No_Mob_BPISI_LSTM prediction')
    plt.legend(loc='best')
    ...
    plt.savefig('../result/figure/India/10/No_Mob_BPISI_LSTM/confirmed_delta_' + country +
                '_' + str(learning_rate) + '_' + str(hidden_num) + '_' + str(epochs) + '_' + str(window_backward) + '.png')
...
if __name__ == '__main__':
    ...
    feature_country, drop_ind = preprocess.merge_hist(mobility, disease, country, mask_ind=None)   # merged features


    train_val_data = feature_country.iloc[:-test_num, :]
    drop_colname = train_val_data.columns[list(range(6, 12))].tolist()
    train_val_data.drop(drop_colname, axis=1, inplace=True)
    ...
    for ind, df in zip(inds, dfs):
        df.drop(drop_colname, axis=1, inplace=True)
        temp = []
        for i in range(3):
            temp.append(df.iloc[:-(ind + uncertain_day - 10 * i), 2:-1].values.reshape(-1, n_features))
        data_set.append(temp)


    # data normalization
    test_X_set = [[min_max_scaler.transform(data[i]) for i in range(3)] for data in data_set]
    # predict the infection rate
    test_pred_set = []
    for data_scaled in test_X_set:
        temp = []
        for i in range(3):
            pre = model_test(LSTM, data_scaled[i])
            if i !=2:
                temp.append(pre)
            else:
                temp.append(pre[:5])
        test_pred_set.append(temp)
    ...
    disease_pre = [pd.read_csv('../result/disease_pre/India/10/' + country + '_' + str(ind) + '.csv', header=0) for ind in inds]
    ...
    with open('../result/figure/India/10/No_Mob_BPISI_LSTM/' + country + '_model_res.txt', 'a') as file:
