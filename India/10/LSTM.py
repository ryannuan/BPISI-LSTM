...
def loss_res(epochs, train_loss, val_loss, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/10/LSTM/loss_' + country +
...
def model_res(test_pred, ind, learning_rate, hidden_num):
    ...
    plt.savefig('../result/figure/India/10/LSTM/confirmed_delta_' + country +
                '_' + str(learning_rate) + '_' + str(hidden_num) + '_' + str(epochs) + '_' + str(window_backward) + '.png')
...
if __name__ == '__main__':
    ...
    pred_rmse = np.mean([rmse(test_pred[i], test_real[i]) for i in range(3)])
    pred_mape = np.mean([mape(test_pred[i], test_real[i]) for i in range(3)])
    pred_mae = np.mean([mae(test_pred[i], test_real[i]) for i in range(3)])
    ...
    with open('../result/figure/India/10/LSTM/' + country + '_model_res.txt', 'a') as file:
